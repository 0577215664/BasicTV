...
void tv_audio_init(){
	if(settings::get_setting("audio") == "true"){
		...
		if(settings::get_setting("audio_playback") == "ao"){
			audio_playback = TV_AUDIO_PLAYBACK_AO;
		}else if(settings::get_setting("audio_playback") == "sdl"){
			audio_playback = TV_AUDIO_PLAYBACK_SDL;
		}else{
			print("invalid audio_playback system, assuming libao", P_WARN);
			audio_playback = TV_AUDIO_PLAYBACK_AO;
		}
		audio_playback_list[audio_playback]->init();
	}else{
...
void tv_audio_close(){
	if(settings::get_setting("audio") == "true"){
		audio_playback_list[audio_playback]->close();
	}
}
