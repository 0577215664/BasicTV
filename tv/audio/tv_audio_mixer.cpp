	...
	for(uint64_t i = 0;i < frame_vector.size();i++){
		...
		std::vector<std::vector<uint8_t> > wave_buffer =
			transcode::audio::frames::to_codec(
				...
				&wav_audio_prop);
		ASSERT(wave_buffer.size() == 1, P_WARN);
		ASSERT(wave_buffer.at(0).size() == 1, P_WARN);
		SDL_RWops *rw =
			SDL_RWFromMem(
				wave_buffer[0].data(),
				wave_buffer[0].size());
		...
		if(std::find_if(
			...
			print("adding and playing audio", P_DEBUG);
			buffer.push_back(
				std::make_tuple(
...
static std::vector<std::pair<id_t_, uint64_t> > tv_audio_mixer_find_frames(){
	...
	for(uint64_t i = 0;i < window_vector.size();i++){
		...
		if(active_streams.size() == 0){
			continue;
		}else if(active_streams.size() > 1){
			print("more than one active stream, probably a bug", P_WARN);
		}
		tv_item_t *item_ptr =
			PTR_DATA(window_ptr->get_active_streams().at(0),
				 tv_item_t);
		...
		if(new_id == ID_BLANK_ID){
			ASSERT(new_id == ID_BLANK_ID, P_DEBUG);
			continue;
		}
		retval.push_back(
